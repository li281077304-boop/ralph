...
export async function runNativeGoalWorker(options: {
  ...
  if (devlogRoot) {
    ...
    devlogEntry = await createDevlogHandoff({
      ...
      context: [
        "USER OBSERVATION",
        "The current task requires a fresh Worker Goal invocation.",
        "CONFIRMED FACT",
        ...
        "A prior conversation or active status alone is not sufficient progress evidence.",
        "DECISION",
        "Persist and validate the handoff before starting or resuming the Goal.",
        "UNKNOWN / OPEN RISKS",
        recent,
      ].join("\n"),
